...
export default function DashboardRep() {
  ...
  const [kpis, setKpis] = useState<Kpis>({
    visitasMes: 0, comissaoPrevista: 0, contasVencendo: 0,
    contasVencendoValor: 0, testesAtivos: 0, pedidosMes: 0,
    pedidosValorMes: 0, ranking: null,
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Só consulta tabelas que existem no banco
        const [{ count: visitasMes }, { data: clientes }, { data: vAll }] = await Promise.all([
          supabase
            .from("nutrir_visitas")
        ...
        setKpis(prev => ({ ...prev, visitasMes: visitasMes ?? 0 }));
        setProximasVisitas(sugeridas);
      } catch (err) {
        console.error("[DashboardRep]", err);
        // kpis já inicializado com zeros, grid continua visível
      }
    })();
  }, [org, user, inicioMes]);
