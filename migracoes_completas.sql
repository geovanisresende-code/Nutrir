...
ON CONFLICT (crop, analysis_type, nutrient) DO NOTHING;-- ============ AI CHAT ============
...
CREATE POLICY "plant_photos_delete" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'plant-photos' AND public.is_org_member((storage.foldername(name))[1]::uuid, auth.uid()));ALTER TABLE public.organizations
...
CREATE INDEX IF NOT EXISTS idx_ndvi_org_date
  ON public.ndvi_readings (organization_id, captured_at DESC);-- Tabela de relatórios
...
CREATE INDEX idx_audit_entity ON public.audit_log (entity_type, entity_id);-- Subscriptions per organization
...
UPDATE public.plans SET
  ...
  stripe_price_id_yearly = CASE tier
    ...
    ELSE NULL END
WHERE tier IN ('pro', 'enterprise');ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS onboarding_completed_at TIMESTAMPTZ;

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS onboarding_step SMALLINT NOT NULL DEFAULT 0;-- Tabela
...
FOR EACH ROW EXECUTE FUNCTION public.notify_on_new_member();-- Cache de cotações
...
EXCEPTION WHEN duplicate_table OR duplicate_object THEN NULL; END $$;INSERT INTO public.nutrir_culturas (nome, nome_cientifico, categoria, ciclo_dias, ativo)
VALUES
...
CREATE TRIGGER nutrir_foliar_hist_touch
  ...
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();-- Histórico de cálculos NPK (drench/fertirrigação/nonino/localizada)
...
FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();ALTER TABLE public.nutrir_pedidos
...
CREATE INDEX IF NOT EXISTS idx_nutrir_pedidos_orcamento_origem
  ON public.nutrir_pedidos(orcamento_origem_id);-- Tabela de auditoria de mudanças de status
...
END $fn$;-- Tabela de solicitações de cadastro (autorização pelo admin)
...
ON CONFLICT (user_id) DO NOTHING;-- ============ RDV (Relatório Diário de Viagem / despesas) ============
...
END $$;CREATE OR REPLACE FUNCTION public.gerar_cr_e_comissao_do_pedido()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  prazo int := 30;
  pct numeric := 0;
...
BEGIN
  ...
  RETURN NEW;
END $$;ALTER TABLE public.nutrir_rdv
  ...
  ADD COLUMN IF NOT EXISTS hotel_nome text;-- Enum de status
DO $$ BEGIN
...
ALTER TABLE public.nutrir_campos_teste
  ADD COLUMN IF NOT EXISTS variedade text;ALTER TABLE public.nutrir_pedidos
...
DO $$ BEGIN
  CREATE POLICY "pedidos_assin_delete" ON storage.objects FOR DELETE
    USING (bucket_id = 'pedidos-assinaturas' AND auth.uid() IS NOT NULL);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;-- Função para updated_at
...
FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();INSERT INTO storage.buckets (id, name, public)
...
TO authenticated
USING (bucket_id = 'produto-imagens');-- Adicionar campos para o novo formulário de pedido
...
FOR EACH ROW EXECUTE FUNCTION public.validate_pedido_tipo_venda();-- Enum dos 6 cargos visíveis ao usuário final
DO $$ BEGIN
